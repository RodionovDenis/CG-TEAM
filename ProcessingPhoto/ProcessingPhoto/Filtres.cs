...
namespace ProcessingPhoto
{
    abstract class Filtres
    {
        ...
        {
            Bitmap resultImage = new Bitmap(sourseImage.Width, sourseImage.Height); //такой же размер
    ...
    class MatrixFilters : Filtres
    {
        ...
        protected override Color calculateNewPixelColor(Bitmap sourseImage, int x, int y) // х и у координаты текущего пикселя
        {
            int radiusX = kernel.GetLength(0) / 2; //ядро фильтра
    ...
    class SharpnessFilter : MatrixFilters
    {
        ...
        {
            kernel = new float[3, 3] { { -1f, -1f, -1f }, { -1f, 9f, -1f }, { -1f, -1f, -1f } };
        }
    }
    ...
    class MaximumFilter : MedianFilter
    {
        private float[] arrayR = null;
        private float[] arrayG = null;
        private float[] arrayB = null;
        ...
        protected override Color calculateNewPixelColor(Bitmap sourseImage, int x, int y)
        {
            ...
            Array.Sort(arrayR); Array.Sort(arrayG); Array.Sort(arrayB);
            return Color.FromArgb(Clamp((int)arrayR[8], 0, 255), Clamp((int)arrayG[8], 0, 255), Clamp((int)arrayB[8], 0, 255));
        }
    }
    class MinimumFilter : MatrixFilters
    {
        private float[] arrayR = null;
        private float[] arrayG = null;
        private float[] arrayB = null;
        ...
        protected override Color calculateNewPixelColor(Bitmap sourseImage, int x, int y)
        {
            ...
            Array.Sort(arrayR); Array.Sort(arrayG); Array.Sort(arrayB);
            return Color.FromArgb(Clamp((int)arrayR[0], 0, 255), Clamp((int)arrayG[0], 0, 255), Clamp((int)arrayB[0], 0, 255));
        }
    }
    class LuminousEdgesFilter : MatrixFilters
    {
        private MedianFilter median = new MedianFilter();
        private PruittFilter pruitt = new PruittFilter();
        private MaximumFilter maximum = new MaximumFilter();
        public LuminousEdgesFilter() { }
        public override Bitmap processImage(Bitmap sourseImage, BackgroundWorker worker)
        {
            Bitmap picture = median.processImage(sourseImage, worker);
            picture = pruitt.processImage(picture, worker);
            return maximum.processImage(picture, worker);
        }
    }
    ...
    class PerfectReflector : Filtres //идеальный отражатель
    {
    ...
    class CorrectionWithAReferenceColor : Filtres //коррекция с опорным цветом
    {
        Color main;
